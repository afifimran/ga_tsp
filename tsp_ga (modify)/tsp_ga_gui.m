...
nn = 10;
tt = [ 4.6464    5.8992    8.8247    4.1658    5.0661    8.9144    3.9393    6.5901    3.1603    3.2214; 
    5.2165    9.9698    1.5920    6.5641    0.3140    6.1154    4.3330    3.3675    0.4730    2.6937];

r=tt; % randomly distribute cities
...
% 
%  r = [ [5, 10], [8, 20], [12, 30], [15, 40], [18, 50], [21, 60], [24, 70], [27, 80], [30, 90], [33, 100] ];
% r = asz*tt;
% r = [1 2; 3 4; 5 6]; 
% r=asz*tt; % randomly distribute cities
% r(1,:) -x coordinaties of cities
% r(2,:) -y coordinaties of cities
...
%         % crossover:
% ii = roulette_wheel_indexes(ps,p); % select parent genes for crossover
% Gc = G(ii,:); % genes to crossover
% Gch = zeros(ps,nn); % childrens
% for prc=1:(ps/2) % pairs counting
%     i1 = 1+2*(prc-1);
%     i2 = 2+2*(prc-1);
%     g1 = Gc(i1,:); % parent 1 gene
%     g2 = Gc(i2,:); % parent 2 gene
%     cp1 = ceil((nn-1)*rand); % first crossover point
%     cp2 = cp1 + ceil((nn-cp1)*rand); % second crossover point
%     
%     % create children:
%     g1ch = g2(cp1:cp2); % take segment from parent 2
%     g2ch = g1(cp1:cp2); % take segment from parent 1
%     for i=1:nn
%         if ~ismember(g1(i),g1ch)
%             g1ch = [g1ch g1(i)]; % add missing elements from parent 1
%         end
%         if ~ismember(g2(i),g2ch)
%             g2ch = [g2ch g2(i)]; % add missing elements from parent 2
%         end
%     end
%     Gch(i1,:) = g1ch;
%     Gch(i2,:) = g2ch;
